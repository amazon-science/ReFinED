...
def main():
    ...
    languages = ['de','es','fr','it'] # we test only 4 languages for the TR2016 dataset
    ...
    for lang in languages:
        ...
        metrics_el = evaluate_on_docs(refined=refined, docs=dataset_docs, dataset_name=dataset_name, 
                                    el=True,
                                    ed_threshold = 0.0,topk_eval=True,top_k=3)
        print('*****************************\n\n')
        print(f'Dataset name: {dataset_name}')
        print(metrics_el.get_summary())
        print('*****************************\n\n')
        all_recall+=metrics_el.get_recall()
