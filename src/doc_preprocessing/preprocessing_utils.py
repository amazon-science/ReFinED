...
def convert_batch_element_to_tensors(
    batch_element: BatchElement, processor: DocPreprocessor
) -> BatchElementTns:
    """
    ...
    if len(batch_element.spans) > 0:
        max_candidates = len(batch_element.spans[0].candidate_entity_ids)
    else:
    ...
    pme_values = torch.zeros((num_ents, max_candidates), dtype=torch.float32)
    ...
    if (
        num_ents > 0
        and batch_element.spans[0].gold_entity_id is None
        and batch_element.spans[0].mention_gold_classes is None
    ) or num_ents == 0:
        ...
        if len(batch_element.spans) > 0 and batch_element.spans[0].gold_entity_id is not None:
            gold_qcode_values = torch.zeros((num_ents,), dtype=torch.long)
        else:
            gold_qcode_values = None

    if has_gold_label:
        # set class targets directly form span if present (0 will be used as padding fill value)
        if len(batch_element.spans) > 0 and batch_element.spans[0].mention_gold_classes is not None:
            # use the provided classes
            fill_value = 0
            n = processor.max_num_classes_per_ent
            class_target_values[:num_ents] = torch.tensor(
                [
                    span.mention_gold_classes + [fill_value] * (n - len(span.mention_gold_classes))
                    for span in batch_element.spans
                ]
            )
        else:
            # gold_qcode_values must be set so use the qcode to get the classes (types)
            gold_qcodes = [span.gold_entity_id for span in batch_element.spans]
    ...
    if num_ents > 0:
        candidate_qcodes = [str(y[0]) for x in batch_element.spans for y in x.candidate_entity_ids]
        ...
        candidate_class_values[
            :num_ents, :max_candidates
        ] = processor.get_classes_idx_for_qcode_batch(
        ...
        candidate_pem_values = np.array(
            [y[1] for x in batch_element.spans for y in x.candidate_entity_ids]
        ).reshape(num_ents, max_candidates)

        # TODO remove pme completely
        candidate_pme_values = np.array(
            [y[1] for x in batch_element.spans for y in x.candidate_entity_ids]
        ).reshape(num_ents, max_candidates)

        if batch_element.spans[0].additional_ed_features is not None:
            candidate_features = torch.tensor(
                [astuple(f) for span in batch_element.spans for f in span.additional_ed_features],
                dtype=torch.float32,
            ).view((num_ents, max_candidates, len(astuple(AdditionalEDFeatures.get_pad_element()))))

        pem_values[:num_ents, :max_candidates] = torch.from_numpy(candidate_pem_values)
        pme_values[:num_ents, :max_candidates] = torch.from_numpy(candidate_pme_values)

    for ent_idx, ent in enumerate(batch_element.spans):
        # must handle None values keep ordering same but with None placeholders
        if has_gold_label:
            gold_qcode = ent.gold_entity_id
            gold_ent_in_cands = False

        for cand_idx, (qcode, pem_value) in enumerate(ent.candidate_entity_ids):
...
def convert_doc_to_tensors(
    ...
    if collate:
        return convert_batch_elements_to_batched_tns(
            doc.to_batch_elements(data_dir=preprocessor.data_dir, override_max_seq=max_seq),
            preprocessor,
        ...
        return [
            ...
            for elem in doc.to_batch_elements(
                data_dir=preprocessor.data_dir, override_max_seq=max_seq
            )
        ]


def collate_batch_elements_tns(
    batch_elements_tns: List[BatchElementTns], token_pad_value: int, ner_pad_value: int = -1
) -> BatchedElementsTns:
    """
    ...
    b_pme_values = torch.zeros((batch_size, b_num_ents, max_candidates), dtype=torch.float32)
    ...
    for item_idx, batch in enumerate(batch_elements_tns):  # TODO replace unpacking
        ...
        b_pme_values[item_idx, :num_ents] = batch.pme_values
    ...
    if batch_elements_tns[0].candidate_features is not None:
        b_candidate_features = pad_sequence(
            [elem.candidate_features for elem in batch_elements_tns], batch_first=True
        )
    else:
        # candidate features not present. add a tensor with zeros instead
        b_candidate_features = torch.zeros(
            size=[
                batch_size,
                b_num_ents,
                max_candidates,
                len(astuple(AdditionalEDFeatures.get_pad_element())),
            ],
            dtype=torch.float32,
            device=b_token_id_values.device,
        )
    ...
    return BatchedElementsTns(
        ...
        b_ner_labels,
        candidate_features=b_candidate_features,
