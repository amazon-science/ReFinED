...
class ELDataset(Dataset):
    """
    This is a dataset reader for ReFinED standard dataset format (generated from .xml format).
    Datasets include: ACE, CWEB, WIKI-WNED.
    """
    ...
    def __init__(
        ...
        random.seed(seed)
        split_to_filename = {"train": "datasets/tac_2010_train_parsed.json"}
    ...
    def from_dataset_mention(
        ...
        wiki_title = dataset_mention["wiki_name"].replace(" ", "_")
        if wiki_title is None or wiki_title == "NIL":
            return None, person_coref
        gold_qcode = self.preprocessor.map_title_to_qcode(title=wiki_title)
        if gold_qcode is None or gold_qcode in self.preprocessor.disambiguation_qcodes:
            return None, person_coref
        start = dataset_mention["start"]
    ...
    def from_dataset_line(self, dataset_line: Dict[str, Any]) -> "Doc":
        ...
        """
        dataset_mentions = dataset_line["mentions"]
        dataset_mentions.sort(key=lambda x: x["start"])
